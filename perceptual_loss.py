# [Source: Open-Sora-Plan] opensora/models/causalvideovae/model/losses/perceptual_loss.py
# [Modified - oliviaa] Changed relative imports to local imports
import torch
from torch import nn
import torch.nn.functional as F
from lpips_local import LPIPS  # [Modified - oliviaa] lpips → lpips_local (패키지 lpips와 충돌 방지)
from einops import rearrange
from discriminator import weights_init, NLayerDiscriminator3D  # [Modified - oliviaa] .discriminator → discriminator
...
class LPIPSWithDiscriminator3D(nn.Module):
    def __init__(
        ...
        self.lpips_chunk_size = 0  # 0=전체 한번에, >0=chunk 단위 순차 계산 (메모리 절약)
    ...
    def forward(
        ...
        if optimizer_idx == 0:
            ...
            if self.perceptual_weight > 0:
                if self.lpips_chunk_size > 0 and inputs.shape[0] > self.lpips_chunk_size:
                    _chunks = []
                    for _i in range(0, inputs.shape[0], self.lpips_chunk_size):
                        _chunks.append(self.perceptual_loss(
                            inputs[_i:_i+self.lpips_chunk_size],
                            reconstructions[_i:_i+self.lpips_chunk_size]))
                    p_loss = torch.cat(_chunks, dim=0)
                else:
