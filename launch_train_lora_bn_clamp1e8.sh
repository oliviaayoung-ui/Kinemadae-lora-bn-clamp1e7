#!/bin/bash
# LoRA + BN 학습, fresh start, adaptive clamp 1e8.
# 1e7 setup 에서 weighted_align grad ≈ rec_grad × 0.54 (rec 2배 큼) — 1:1 만들기 위해 풀기.
# 예상: hit 비율 60% → ~30%, ratio 0.54 → ~0.75.
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name kinemadae_lora_bn_clamp1e8 \
